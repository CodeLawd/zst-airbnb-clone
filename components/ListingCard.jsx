...
const ListingCard = ({ listing: { images, name, dateAvailable, rating, cost, host }, listingId }) => {
  ...
  const isImage = () => setLoading(false);

  return (
    <div>
      {loading && <SkeletonListing />}
      <div className={`${loading ? "invisible" : "visible"} mb-5 group cursor-pointer`}>
        <div className="w-full h-[180px] md:h-[260px] relative overflow-hidden flex rounded-lg">
          {images.map((img, i) => (
            <img
              loading="lazy"
              onLoad={i == 0 ? isImage : null}
              key={i}
              src={img}
              alt={img}
          ...
          <div className="absolute flex gap-1 left-[45%] bottom-4">
            {images.map((img, i) => (
